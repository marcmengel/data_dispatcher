...
class Handler(BaseHandler):
        ...
        for f in files:
            if isinstance(f, str):
                try:
                    ...
                    return (
                        400,
                        f"Invalid file specification: {f} - can not parse as namespace:name",
                    )
                ...
                if "namespace" not in f or "name" not in f:
                    return (
                        400,
                        f"Invalid file specification: {f} - namespace or name missing",
                    )
                attrs = f.get("attributes", {})
                if not isinstance(attrs, dict):
                    return (
                        400,
                        f"Invalid file specification: {f} - invalid file attributes specification, expected a dictionary",
                    )
    ...
    def restart_project(
        self, request, relpath, project_id=None, force="no", failed_only="yes", **args
    ):
        force = force == "yes"
    ...
    def copy_project(self, request, relpath, **args):
        ...
        for h in original_project.handles():
            ...
            files_updated.append(
                dict(namespace=h.Namespace, name=h.Name, attributes=attrs)
            )
        ...
        project.add_log(
            ...
            event="copied",
            source=project_id,
            override=dict(project=project_attrs, file=file_attributes),
        )
        self.App.project_created(project.ID)
        return json.dumps(project.as_jsonable(with_handles=True)), "text/json"
    ...
    def next_file(
        self, request, relpath, project_id=None, worker_id=None, cpu_site=None, **args
    ):
        # print("next_file...")
        user, error = self.authenticated_user()
        if user is None:
        ...
        if handle is None:
            ...
            out = {"handle": info, "reason": "reserved", "retry": False}
        return json.dumps(out), "text/json"

    def release(
        self, request, relpath, handle_id=None, failed="no", retry="yes", **args
    ):
        if handle_id is None:
            return (
                400,
                "File Handle ID (<project_id>:<namespace>:<name>) must be specified",
            )
        user, error = self.authenticated_user()
        if user is None:
            return 401, error
    ...
    def project_files(
        self, request, relpath, project_id=None, state=None, ready_only="no", **args
    ):
        ...
        return (
            json.dumps([h.as_jsonable(with_replicas=ready_only) for h in handles]),
            "text/json",
        )
    ...
    def project(
        ...
        **args,
        ...
        jsonable = project.as_jsonable(
            with_handles=with_files, with_replicas=with_replicas
        )
    ...
    def projects(
        ...
        **args,
        ...
        db = self.App.db()
        projects = DBProject.list(
            db, state=state, not_state=not_state, owner=owner, attributes=attributes
        )
        return (
            json.dumps(
                [
                    p.as_jsonable(
                        with_handles=with_handles, with_replicas=with_replicas
                    )
                    for p in projects
                ]
            ),
    ...
    def file_handle(
        ...
        **args,
        ...
        if handle_id:
            ...
            handle = DBFileHandle.get_for_project(
                db, int(project_id), file_namespace=namespace, file_name=name
            )
        else:
            return (
                400,
                "Either file_id, or file_name='namespace:name' or file_namespace and file_name must be specified",
            )
        if not handle:
            return "null", "text/json"
...
class App(BaseApp, Logged):
    ...
    def proximity_map(self):
        db = self.db()
        return DBProximityMap(
            db, defaults=self.ProximityMapDefaults, overrides=self.ProximityMapOverrides
        )
